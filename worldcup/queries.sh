...
echo "$($PSQL "select cast(total as decimal)/matches as average from (select (SUM(winner_goals) + SUM(opponent_goals)) as total, count(*) as matches from games) as tmp")"
...
echo "$($PSQL "select name from games right join teams on games.winner_id=teams.team_id OR games.opponent_id=teams.team_id where games.year=2014 AND games.round='Eighth-Final' ORDER by name")"
